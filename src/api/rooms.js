...
class Rooms extends UsersController {
	...
	getRoomMessages = asyncWrapper(async (req, res) => {
		...
		const messagesCountData = await db.query(
		...
		if (messagesCount === 0)
			return res.status(200).json({ messages: [], isFinished: true });
	...
	createRoom = asyncWrapper(async (req, res) => {
		...
		if (createdRoom.length <= 0)
			throw new BadRequestError("cannot create the room");
		res.status(201).json({
			createdRoom: createdRoom[0],
	...
	getRooms = asyncWrapper(async (req, res) => {
		...
		let modifiedRooms = [];
		rooms.map(room => {
			const sameRoomIndex = modifiedRooms.findIndex(
				modifiedRoom => modifiedRoom.room_id === room.room_id
			);
			if (sameRoomIndex !== -1) {
				return modifiedRooms[sameRoomIndex].notificationsIds.push(
					room.notification_id
				);
			}
			modifiedRooms.push({
	...
	addNewUser = asyncWrapper(async (req, res) => {
		...
		const roomId = req.params.roomId;
		if (!roomId || !userId)
			throw new BadRequestError("parameters are required");
		...
		if (isAlreadyJoinedData.length > 0)
			throw new BadRequestError("you cant add the user twice");
